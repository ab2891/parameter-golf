    ...
    def _run_layers(self, x: Tensor, x0: Tensor, token_ids: Tensor | None = None) -> Tensor:
        ...
        for i in range(num_enc):
            if self.level_signals is not None:
                x = x + self.level_signals[i](x)
            x = self._get_block_for_layer(i)(x, x0, self.attn_norms[i], self.mlp_norms[i])
            skips.append(x)
...
def main() -> None:
    ...
    if args.progressive_loop_start > 0 and args.num_loops > 2:
